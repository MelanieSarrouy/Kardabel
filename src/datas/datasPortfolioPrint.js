import mascaradesCover1 from '../assets/images/portfolioGraphisme/affiche-Mascarades2019.png'
import mascaradesCover2 from '../assets/images/portfolioGraphisme/affiche-Mascarades2018.png'
import mascaradesFlyer2019 from '../assets/images/portfolioGraphisme/mockup_fly_mascarades2019.png'
import mascaradesFlyer2018 from '../assets/images/portfolioGraphisme/mockup_fly_mascarades2018.png'
import follesbergeresCover1 from '../assets/images/portfolioGraphisme/affiche-FollesBergeres2020.png'
import follesbergeresCover2 from '../assets/images/portfolioGraphisme/affiche-FollesBergeres2019.png'
import follesbergeresFlyer2020 from '../assets/images/portfolioGraphisme/mockup_fly_follesBergeres2020.png'
import follesbergeresFlyer2019 from '../assets/images/portfolioGraphisme/mockup_fly_follesBergeres2019.png'


export const datasPortfolioPrint = [
  {
    title: 'Les Mascarades Médiévales',
    dates: [2017, 2018, 2019],
    description:
      "Organisation complète de l'évènement \n Conception et réalisation de tous les supports de communication",
    client: 'Mairie de La Couvertoirade',
    images: {
      cover1: mascaradesCover1,
      cover2: mascaradesCover2,
      more: [mascaradesFlyer2019, mascaradesFlyer2018],
    },
    tags: ['affiche', 'flyer', 'bannière réseaux sociaux'],
  },
  {
    title: 'Les Folles Bergères',
    dates: [2019, 2020],
    description:
      "Organisation complète de l'évènement \n Conception et réalisation de tous les supports de communication",
    client: 'Mairie de La Couvertoirade',
    images: {
      cover1: follesbergeresCover1,
      cover2: follesbergeresCover2,
      more: [follesbergeresFlyer2020, follesbergeresFlyer2019],
    },
    tags: ['affiche', 'flyer', 'bannière réseaux sociaux'],
  },
  {
    title: 'Les Ailes du Larzac',
    dates: [2019, 2020, 2021],
    description: 'Conception et réalisation des supports de communication',
    client: 'Les Ailes du Larzac',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['affiche', 'flyer', 'banderole', 'autocollant pour véhicule'],
  },
  {
    title: 'Brelock en Stock',
    dates: [2019, 2020],
    description: "Conception et réalisation d'une affiche",
    client: 'APE',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['affiche'],
  },
  {
    title: 'Tournois',
    dates: [2022],
    description:
      'Exercices de conception et réalisation de templates pour des affiches de tournois',
    client: '-',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['affiche'],
  },
  {
    title: 'Visites Guidées à La Couvertoirade',
    dates: [2017, 2018, 2019, 2020, 2021, 2022],
    description:
      'Conception et réalisation des brochures de présentation des visites guidées pour les groupes adultes et scolaires à La Couvertoirade \n (format carré - 12 pages - broché)',
    client: 'Service Tourisme de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['brochure'],
  },
  {
    title: 'Plaquette de présentation',
    dates: [2020],
    description:
      'Conception et réalisation de la plaquette de présenation de Kardabel v.1 \n (format A5 - 2 volets - 1 pli roulé)',
    client: 'Kardabel (v.1)',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['dépliant'],
  },
  {
    title: 'Programme de la saison',
    dates: [2018, 2019, 2020],
    description:
      'Conception et réalisation du programme de la saison touristique à La Couvertoirade \n Programmation et mise en place des animations en 2018 et 2019 \n (format A5 - 2 volets - 1 pli roulé)',
    client: 'Mairie de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['dépliant'],
  },
  {
    title: "Le Trésor d'Offerus",
    dates: [2020],
    description:
      "Conception et réalisation d'un livret pour un jeu de piste \n Co-création du contenu \n (format A5 - 36 pages - broché )",
    client: 'Service Tourisme de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['livret'],
  },
  {
    title: 'Le Festival des Épouvantails',
    dates: [2020],
    description:
      "Conception et réalisation des supports de communication \n Co-organisation de l'évènement",
    client: 'Association Larzac Repère',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['affiche', 'flyer', 'bannière réseaux sociaux'],
  },
  {
    title: 'Insonnance 01',
    dates: [2019],
    description: 'Conception et réalisation du macaron du 1er vinyle édité par Vinylbleu.fr',
    client: 'Association Larzac Repère',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['macaron'],
  },
  {
    title: 'Faire-part',
    dates: [2019],
    description:
      'Conception et réalisation de faire-part de naissance, mariage, invitations, etc. \n (format carré - 2 volets - 1 pli roulé)',
    client: 'Particuliers',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['carte', 'dépliant'],
  },
  {
    title: 'Pierre & Sophie',
    dates: [2020, 2021],
    description:
      "Conception et réalisation de 4 déclinaisons d'étiquettes pour la vente de produits alimentaires de production artisanale \n (diamètre 6 cm) \n Conception et réalisation du 'logo' pour les signatures de mail et le papier d'emballage alimentaire",
    client: 'GAEC des Infruts',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['étiquette', 'logo'],
  },
  {
    title: 'Larzac Repère',
    dates: [2021, 2022],
    description:
      "Conception et réalisation de l'identité visuelle et des supports de communication de l'association",
    client: 'Association Larzac Repère',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['logo', 'carte'],
  },
  {
    title: 'La Couvertoirade',
    dates: [2016, 2017, 2018, 2019, 2020, 2021],
    description:
      'Conception et réalisation du dépliant de présentation de la Cité médiévale de La Couvertoirade \n (format DL - 3 volets - 2 plis roulés)',
    client: 'Service Tourisme de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['dépliant'],
  },
  {
    title: "L'Abat'Mobile 34",
    dates: [2022],
    description:
      "Conception et réalisation du dépliant de présentation du projet d'abattage à la ferme \n (format DL - 3 volets - 2 plis roulés) \n Conception et réalisation du logo et de ses déclinaisons \n Conception et réalisation d'un site vitrine pour le projet : www.labatmobile34.fr ",
    client: "Association Abattage à la Ferme de l'Hérault",
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['dépliant', 'logo', 'site'],
  },
  {
    title: 'Panneaux',
    dates: [2016, 2018, 2022],
    description: 'Conception et réalisation de panneaux informatifs intérieur et extérieur',
    client: 'Service Tourisme de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['panneaux'],
  },
  {
    title: 'Kakemonos',
    dates: [2016, 2019],
    description: 'Conception et réalisation de kakemonos (Roll Up)',
    client: 'Service Tourisme de La Couvertoirade',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['kakemono'],
  },
  {
    title: 'Cartes',
    dates: [2020, 2021],
    description:
      "Conception et réalisation de cartes de visites, cartes d'adhésion, carte de fidélité, etc. \n (format 85 x 55 mm - coins arrondis - vernis sélectif)",
    client: 'divers',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['carte'],
  },
  {
    title: 'Logos',
    dates: [2016, 2020, 2021, 2022],
    description:
      "Conception et réalisation de logos, avec ou sans déclinaisons",
    client: 'divers',
    images: {
      cover1: '',
      cover2: '',
      more: ['', '', ''],
    },
    tags: ['logo'],
  },

]
